...
export default function BrokerConnectModal({ onClose }: Props) {
  ...
  async function handleConnect() {
    ...
    try {
      ...
      // Network error — still try to save
    }
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center">
        <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={skipAndGo} />
        ...
        <div className="relative w-full max-w-lg rounded-[32px] border border-cyan-400/20 bg-[linear-gradient(180deg,rgba(6,18,42,0.98),rgba(4,12,28,0.98))] p-8 shadow-[0_20px_80px_rgba(0,0,0,0.6)] backdrop-blur-xl">
          ...
          {status === "connected" ? (
            <div className="mt-6 space-y-4">
              ...
              <button
                onClick={() => { setHidden(true); onClose?.(); window.location.href = "/dashboard"; }}
            ...
            <div className="mt-6 space-y-4">
              ...
              {/* How to connect guide button */}
              <button
                onClick={() => setShowGuide(true)}
                className="flex w-full items-center justify-center gap-2 rounded-2xl border border-cyan-400/15 bg-cyan-400/5 px-4 py-3 text-sm font-medium text-cyan-300 transition hover:bg-cyan-400/10"
              >
                <span>&#128214;</span>
