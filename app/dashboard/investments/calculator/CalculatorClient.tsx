...
interface CalculatorClientProps {
  initialStocks: Array<{ symbol: string; company_name: string; source?: string }>
  initialMode: string
}

export default function CalculatorClient({ initialStocks, initialMode }: CalculatorClientProps) {
  ...
  const [watchlistStocks, setWatchlistStocks] = useState<any[]>(initialStocks)
  const [brokerMode, setBrokerMode] = useState(initialMode)
  const [showPeaks, setShowPeaks] = useState(false)
  const [peaks, setPeaks] = useState<any[]>([])
  const [pullbacks, setPullbacks] = useState<any[]>([])

  // Watchlist data is now passed as props from server component
